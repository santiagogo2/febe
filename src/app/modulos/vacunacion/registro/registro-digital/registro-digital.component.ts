...
export class RegistroDigitalComponent implements OnInit {
	...
	keyEvent( event: KeyboardEvent ) {
		this.clickFlag = true;
		if( this.scannerData.length > 20 ) {
			this.clickFlag = false;
		}
		if( event.key && event.key != 'undefined' && event.key != 'null' && event.key != 'Shift' && event.key != 'Enter' && event.key != 'Tab' ) {
	...
	searchThird( searchForm ) {
		if( searchForm.valid && this.clickFlag ) {
	...
	downloadFile() {
		...
		this.vacunadoService.downloadTrainingDocument( this.vaccinated.archivo ).subscribe(
			res => {
	...
	deleteFile(loadedDocument) {
		this.vacunadoService.deleteFile( loadedDocument ).subscribe();
	}
	...
	setInitialVaccinated( tipoUsuarioVacunado = null ) {
		...
		this.vaccinated = new Vaccinated( null, this.globalService.setMaxDate(), tipoUsuarioVacunado, null, null, null, null, null, null, null,
										  null, null, null, null, null, null, null, null, null, 'NINGUNO', 'NINGUNO', false, false,
										  ...
										  null, null, null, null, null, null, 'NINGUNO', null, null)
	}
}
